...
namespace TPL.PVZR.ViewControllers.Entities.Zombies.View.ViewController
{
    ...
    /// 仅适用：气球僵尸
    /// </summary>
    public class DiggerZombieViewController : ZombieViewController
    {
        ...
        protected override void SetUpFSM()
        {
            ...
            ViewFSM.State(ZombieState.Digging_DiggerZombie)
                .OnEnter(() => { _Animator.SetInteger("ZombieState", (int)ZombieState.Digging_DiggerZombie); });
            ViewFSM.State(ZombieState.Excavating_DiggerZombie)
                .OnEnter(() => { _Animator.SetInteger("ZombieState", (int)ZombieState.Excavating_DiggerZombie); });
        ...
        public void Trigger_OnExcavateComplete()
        {
            if (Zombie.FSM.CurrentState is ExcavatingState_DiggerZombie diggingState)
            {
                diggingState.Trigger_OnAnimationEnd();
            }
            else
            {
                $"FSM.CurrentState并非DiggingState_DiggerZombie，而是: {Zombie.FSM.CurrentStateId}".LogError();
            }
        }
    }
}