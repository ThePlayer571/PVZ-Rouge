...
- [ ] `=====当前大任务=====`
- [ ] Award的持久化存储（包括可以配置初始Award）。改为可读取有n个award，然后加载。
- [ ] 新建Item层级，只阻挡植物种植，无碰撞
- [ ] 水路
- [ ] ZombieAI的重构
- [ ] 夜晚
- [ ] 长崎素世流汗和生气表情包
...
- [ ] `=====ZombieAISystem=====`
- [ ] 重写寻路系统，这一版还是有很多遗憾的地方
- [ ] 断梯子的支持
- [ ] 僵尸会跑酷
- [ ] 重构Ai'的passHeight：必须把数据结构改了，一是代码好，二是能确保之前的错误代码全部删除
- [ ] 地图烘焙的逻辑重写，按照vertexType来分而不是MoveType
- [ ] 地形破坏后的重新烘焙
- [ ] 僵尸脱离原有路径的重新寻路
- [ ] `=====阶段任务=====`
- [ ] 完成主流程（第一章）
...
戴夫是无敌的
2K分辨率会使UI错位
僵尸死亡过快会导致游戏无法结束（请按"Pos1生成僵尸"再将其杀掉解决）
