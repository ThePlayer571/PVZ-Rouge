...
- [ ] 主菜单
    ...
    - [ ] 植物图鉴
      ...
      - [ ] 鼠标放在View上，会显示数据
- [ ] CardDefinition改位置
- [ ] NewPapaerZombie：stunned会暂停animator.NewpaperKilled
...
```
帮我拓展编辑器：
看我给出的两个json文件，它们分别对应了PlantId和PlantBookId的每个字段。我经常会忘记设置某些值。
给我做个功能，能检查是不是每个字段都设置了。
预期工作流：
1. 我在unity编辑器打开“CheckJsonConfig”
2. 配置默认的文件路径：PlantValueList.json / PlantBookValueList.json
3. 检查文件是否完备
4. 做出提示
。
你应该把工作流明确地标注出来，以便所有人使用
```
