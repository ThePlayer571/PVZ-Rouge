...
- [ ] Service改成类
- [ ] Phase加lock
- [ ] 加载存档的service，然后Defeat加载存档
- [ ] 把PhaseModel改造成Service
- [ ] 卸载场景的时间改进
...
- [ ] 失败界面
- [ ] 戴夫失败
- [ ] 引入Service层级，审视所有ServiceEvent和部分Command，强阶段Service
...
```
我想为QFramework新建一个层级：Service
public interface IService: ICanInit, ICanGetModel, ICanGetSystem, ICanSendEvent这么多功能

Service是一个类，提供一系列方法，意义是“某个游戏操作的唯一入口”。用IOC存储在Archi里面，和System类似，随用随取。

可以获取Service的有Command Controller System

不要修改QF源文件的任何代码！唯一允许的修改是把一些类/接口改成partial
```
